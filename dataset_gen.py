...
def get_cough_windows(data_folder, fn, window_len, aug_factor=1):
    ...
    - fn: file name of the ground_truth.pkl file listing cough locations
    ...
    - audio_data: NxMx2 data matrix where 
        - N = number of coughs x augmentation factor
        - M = int(window_len * 16000) 16kHz sample freq
    ...
    sig_start = loaded_dict["signal_time"][0]
    sig_end = loaded_dict["signal_time"][1]
    ...
    sound = fn.split('/')[-2]
    fs, air, skin = load_audio(data_folder, subj_id, trial, mov, noise, sound, sig_start, sig_end)
    imu = load_imu(data_folder, subj_id, trial, mov, noise, sound, sig_start, sig_end)
    ...
    for trial in Trial:
        for mov in Movement:
            for noise in Noise:
                ...
                sound = Sound.COUGH
                path = data_folder + subj_id + '/' + subj_id + '_' + trial + '/mov_' + mov + '/background_noise_' + noise + '/' + sound
                if (len(os.listdir(path)) > 0) & os.path.isfile(path + '/ground_truth.pkl'):
                    fn = path + '/ground_truth.pkl'
#                     print(fn)
